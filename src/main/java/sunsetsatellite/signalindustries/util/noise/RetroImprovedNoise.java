...
public class RetroImprovedNoise
	extends BaseImprovedNoise
{
	...
	public void add(double[] arr, double x, double y, double z, int xSize, int ySize, int zSize, double xScale, double yScale, double zScale, double levelScale)
	{
		...
		for(int ix = 0; ix < xSize; ++ix)
		{
			...
			int Xb = Xi & 255;
			X -= Xi;
			double fadeX = X * X * X * (X * (X * 6 - 15) + 10);
			...
			for(int iz = 0; iz < zSize; ++iz)
			{
				...
				int Zb = Zi & 255;
				Z -= Zi;
				double fadeZ = Z * Z * Z * (Z * (Z * 6.0D - 15.0D) + 10.0D);

				for(int iy = 0; iy < ySize; ++iy)
				{
					double Y = (y + (double)iy) * yScale + this.yo;
					int Yi = (int)Y;
					if(Y < Yi)
					{
						--Yi;
					...
					int Yb = Yi & 255;
					Y -= Yi;
					double fadeY = Y * Y * Y * (Y * (Y * 6.0D - 15.0D) + 10.0D);
					if (iy == 0 || Yb != var22)
					{
						var22 = Yb;
						...
						var29 = this.lerp(
								fadeX,
								this.grad(this.p[var65], X, Y, Z),
								this.grad(this.p[var68], X - 1, Y, Z)
						);
						var31 = this.lerp(
								fadeX,
								this.grad(this.p[var66], X, Y - 1, Z),
								this.grad(this.p[var69], X - 1, Y - 1, Z)
						);
						var33 = this.lerp(
								fadeX,
								this.grad(this.p[var65 + 1], X, Y, Z - 1),
								this.grad(this.p[var68 + 1], X - 1, Y, Z - 1)
						);
						var35 = this.lerp(
								fadeX,
								this.grad(this.p[var66 + 1], X, Y - 1, Z - 1),
								this.grad(this.p[var69 + 1], X - 1, Y - 1, Z - 1)
						);
					}

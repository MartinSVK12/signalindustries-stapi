...
public abstract class TieredMachineSimpleFuelBlockEntity extends TieredMachineBaseBlockEntity {
    ...
    public boolean fuel(){
        int burn = getItemBurnTime(itemContents[energySlot]);//SignalIndustries.getEnergyBurnTime(fluidContents[energySlot]);
        if(burn > 0 && canProcess()){
            progressMaxTicks = (int) (200 / speedMultiplier);
            fuelMaxBurnTicks = fuelBurnTicks = burn;
            if(itemContents[1].getItem().hasCraftingReturnItem()) {
                itemContents[1] = new ItemStack(itemContents[1].getItem().getCraftingReturnItem());
            } else {
                itemContents[1].count--;
                if(itemContents[1].count == 0) {
                    itemContents[1] = null;
                }
            }
            return true;
    ...
    private int getItemBurnTime(ItemStack stack) {
        return stack == null ? 0 : FuelRegistry.getFuelTime(stack);
    }

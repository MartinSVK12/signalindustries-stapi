...
import sunsetsatellite.signalindustries.block.AshenTreeLogBlock;
import sunsetsatellite.signalindustries.data.SIBlocks;

import java.util.Random;

public class AshenTreeFeature extends Feature {
    ...
    public AshenTreeFeature(int leavesID, int logID) {
        this.leavesID = leavesID;
        this.logID = logID;
        this.heightMod = 0;
    }

    public AshenTreeFeature(int leavesID, int logID, int heightMod) {
        this.leavesID = leavesID;
        this.logID = logID;
    ...
    void placeLeaves(int i, int j, int k, float f, byte byte0, int leavesId) {
                ...
                while(true) {
                    ...
                    if (d > (double)f) {
                        ...
                        if (i2 != 0 && i2 != this.leavesID) {
                            ...
                            this.world.setBlock(ai1[0], ai1[1], ai1[2], leavesId);
                            ++l1;
                        }
                    }
                }
    ...
    void placeWood(int[] ai, int[] ai1, int logId) {
        ...
        if (ai2[j] != 0) {
            ...
            for(int l = ai2[j] + byte3; k != l; k += byte3) {
                pos[j] = MathHelper.floor((double)(ai[j] + k) + 0.5);
                pos[byte1] = MathHelper.floor((double)ai[byte1] + (double)k * d + 0.5);
                pos[byte2] = MathHelper.floor((double)ai[byte2] + (double)k * d1 + 0.5);
                this.world.setBlock(pos[0], pos[1], pos[2], logId);
                BlockState state = this.world.getBlockState(pos[0], pos[1], pos[2]);
                world.setBlockState(pos[0], pos[1], pos[2], state.with(AshenTreeLogBlock.CONTAINS_RESIN, true));
            }
    ...
    boolean func_519_e() {
        ...
        int i = this.world.getBlockId(this.basePos[0], this.basePos[1] - 1, this.basePos[2]);
            ...
            for (int x = -3; x < 2; x++) {
                for (int z = -3; z < 2; z++) {
                    if(random.nextInt(2) == 0){
                        world.setBlock(this.basePos[0]+x,this.basePos[1]-1,this.basePos[2]+z,  SIBlocks.rootedFabric.id);
                    }
                }
            }
            this.func_521_a();
